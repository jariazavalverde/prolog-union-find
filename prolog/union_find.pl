...
%
% This predicate makes a new set in +UnionFindIn by creating a new element with a unique id, a rank of 0, and a parent
% pointer to itself. The parent pointer to itself indicates that the element is the representative member of its own set.
